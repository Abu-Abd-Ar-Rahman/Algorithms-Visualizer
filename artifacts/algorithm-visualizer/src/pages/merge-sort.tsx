...
function createRun(
  createArray: MergeSortPageProps['createArray'],
  size: number,
) {
  ...
  return {
    initialValues,
    steps: generateMergeSortSteps(initialValues),
  };
}
...
export default function MergeSortPage({
  ...
  const reset = useCallback(
    (size = arraySize) => {
      setRun(createRun(createArray, size));
    },
      ...
      <main className="mx-auto max-w-[1320px] px-5 pb-14 pt-9 sm:px-8 lg:px-10 lg:pt-14">
        <div className="mb-9 max-w-3xl animate-rise-in">
          ...
          <p className="mt-5 max-w-xl text-sm leading-7 text-[hsl(var(--muted-foreground))] sm:text-base">
            A live, step-by-step look at Merge Sort. Follow each split,
            comparison, and write as smaller sorted pieces become one
            complete array.
          </p>
        </div>
          ...
          <div className="min-w-0 space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              <MetricCard
                ...
                detail={
                  totalComparisons
                    ? `${Math.round((playback.comparisons / totalComparisons) * 100)}%`
                    : '—'
                }
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span>
                  {playback.status === 'complete'
                    ? 'Array sorted. Every smaller merge is now part of one ordered result.'
                    : playback.status === 'paused'
                      ? 'Playback is paused. Resume whenever you are ready.'
                      : playback.status === 'running'
                        ? 'Following the divide-and-conquer process…'
                        : 'Ready to split the array into smaller sorted pieces.'}
                </span>
              </div>
        ...
        <section className="mt-12 grid gap-5 border-t border-[hsl(var(--border))] pt-8 sm:grid-cols-[1fr_auto] sm:items-start">
          <div>
            ...
            <p className="max-w-2xl text-lg font-semibold leading-relaxed tracking-[-.03em] text-[hsl(var(--foreground))]">
              “Divide until every piece is simple, then merge the pieces in
              order until the whole array is sorted.”
            </p>
          </div>
          <button
            type="button"
